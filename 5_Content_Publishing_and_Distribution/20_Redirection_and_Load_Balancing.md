...
naver.com이 라운드 로빈 주소 순환을 사용한다고 했을 때 1번에서 2번그림은 주소 순환이 정상적으로 잘 이루어졌다. (210.89.164.90에서 21.89.160.88로 순서대로 넘어갔기 때문에) 하지만 문제는 그 다음 룩업에서 일어나고 말았다. 이번에는 라운드 로빈에 의해 끝자리가 142인 IP로 갈 차례였지만 순서를 하나 건너뛰고 끝자리가 141인 IP로 넘어가버렸다. 그 이유는 무엇일까?
