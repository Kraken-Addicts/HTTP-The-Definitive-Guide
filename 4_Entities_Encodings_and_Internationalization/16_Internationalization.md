...
> **보기**
>
> `문자`, `글리프(glyph)`,` 코딩된 문자(coded character)`, `코드 공간(coding space)`,` 코드 너비(code width)`, `사용 가능 문자집합(character repertoire)`, `코딩된 문자집합(coded character set)`, `문자 인코딩 구조`
...
|                | 문자 코드 값으로 사용하려고 계획해 둔 정수의 범쉬            |
|                | 사용 가능 문자집합(세상의 모든 글자에서 일부분을 선택한 것)을 받아서 각 글자에 코드 공간의 코드를 할당해주는 코딩된 문자들의 집합 |
...
3. 다음 보기에서 설명에 알맞을 것을 골라 채우세요.

> **보기**
>
> `글리프(glyphs)`, `연자(ligatures)`, `문자`

a. `________`은/는 유일하고 추상화된 언어의 요소다

b. `________`은/는 미적인 양식과 스크립트에 따라 각 글자를 그리는 특정한 방법이다

c. `________`은/는 인접한 글자들을 부드럽게 이어주는 것으로 영어에서는 보통 F와 I를 연결한다.
...
1. 다음 보기의 용어로 설명에 알맞게 빈칸을 채우세요. **p.434-435**
...
| 문자               | 알파벳 글자, 숫자, 구두점, 표의문자(중국어에서와 같은), 기호 등 글쓰기의 최소 단위 |
...
3. 다음 보기에서 설명에 알맞을 것을 골라 채우세요. **p.437-438**

a. `문자`은/는 유일하고 추상화된 언어의 요소다

b. `글리프(glyphs)`은/는 미적인 양식과 스크립트에 따라 각 글자를 그리는 특정한 방법이다

c. `연자(ligatures)`은/는 인접한 글자들을 부드럽게 이어주는 것으로 영어에서는 보통 F와 I를 연결한다.
