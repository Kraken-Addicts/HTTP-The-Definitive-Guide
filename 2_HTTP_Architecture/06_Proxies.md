...
![](https://images.velog.io/images/jehjong/post/2013d7ae-b74d-4778-9317-0a6318ca4a67/image.png)
   ...
   c. `kukim`은 GIF를 더 빠르게 다운받거나 압축, 화질 등의 변경을 해주어 더 나은 성능을 제공하는 트랜스코딩 프락시 사이트에 돈을 지불했고 그 결과  `yeha`에게 적절한 GIF를 제 때 보낼 수 있게 되었다.
...
2. 다음 보기에서 골라 빈칸을 채우세요.
   ...
   **<보기>** `콘텐츠 라우터`  `보안 방화벽`  `웹캐시`  `트랜스코더`  `문서 접근 제어자`  `어린이 필터`  `대리 프락시`  `익명화 프락시`  
...
3. `_________` 은/는 HTTP 메시지에서 신원을 식별할 수 있는 특성들을 제거함으로써 개인 정보 보호와 익명성 보장에 기여한다. 개인 정보 보호를 위해 다음 사용자의 메시지에서 무엇을 제거해야하는지 고르세요. (헤더 내용의 일부만 제거 가능)
...
4. 응답 Via 헤더는 보통 요청 Via 헤더와 반대다. (O/X)  
...
4. 응답 Via 헤더는 보통 요청 Via 헤더와 반대다. (O)  
  => p.175 그림 참조   
