...
### 6.6 　  메시지 추적　 `yeha`
1. 
  a. Via 헤더 필드는 개행으로 구분된 경유지(waypoint)의 목록이다. (O/X)
  b. 모든 Via waypoint는 4개의 구성 요소(프로토콜 이름, 프로토콜 버전, 노드 이름, 코멘트)로 구성되어 있다. (O/X)
<details>
<summary> <b> :page_facing_up: 답지 </b>  </summary>
<div markdown="1">
  
1.
  a. Via 헤더 필드는 개행으로 구분된 경유지의 목록이다. (X)
    => 쉼표로 구분한다
  b. 모든 Via waypoint는 4개의 구성 요소(프로토콜 이름, 프로토콜 버전, 노드 이름, 코멘트)로 구성되어 있다. (X)
