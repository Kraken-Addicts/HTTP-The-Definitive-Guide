...
### 9.1 　  크롤러와 크롤링　 
#### 처음 - 9.1.4　 `daelee`
1. 웹 크롤러는 `______`으로 반복하는 방식으로 웹을 순회하는 로봇이다.

   

2. 아래 그림의 웹을 순회하려는 크롤러가 있다. 이 크롤러의 루트집합에 어떤 페이지를 포함시키면 좋을까?

   ![img](https://feel5ny.github.io/images/2019/33/02.png)

3. 크롤러가 웹 페이지에서 절대링크를 발견한 경우 상대링크로 변환할 필요가 있다. **(O / X)**

   

4. 순환은 크롤러에게 필수적이다 **(O / X)**
