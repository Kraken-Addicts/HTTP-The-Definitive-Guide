...
### __13.2.9__ 　 상호 인증
  ...
  **7)** 다음은 **상호 인증(Symmetric Authentication)**에 대한 설명으로 **틀린 것은**? 
  ...
  - 클라이언트 쪽에서도 서버를 인증할 수 있도록 `클라이언트` 가 `난스(=c난스)`를 제공하고, 서버는 이 요약을 `Authentication-Info 헤더`를 통해 클라이언트에 전달하여 인증 받는 것이다.

  - `응답 다이제스트`는 메시지 본문 정보가 다르다는 것만 제외하면 `요청 다이제스트`와 같은 방법으로 계산할 수 있다.
...
- 재전송 공격(Replay Attack)
  - 메시지를 복사한 후 재전송함으로써 승인된 사용자로 오인하게 만들어 공격
