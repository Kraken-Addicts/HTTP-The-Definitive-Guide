...
4. 게이트웨이는 스스로가 리소스를 보유한 진짜 서버인 것처럼 요청을 다뤄 클라이언트는 상대가 게이트웨이인 것을 알 수 없을 때도 있다고 합니다. 하지만 항상 알 수 없는 것은 아닌 것 같습니다. (X)

5. 터널은 HTTP가 아닌 데이타도 전송할 수 있어 웹트래픽만 허용하는 사내 방화벽도 통과할 수 있습니다. (O)

6. 웹브라우저는 웹 요청을 만드는 애플리케이션으로 HTTP 에이전트입니다. (O)
